...
from tensorflow.keras.layers import Input, Dense, Lambda, Subtract, Add, Reshape, Conv2D, Multiply
...
class NBeatsNet:
    ...
    def __init__(self,
        ...
        self.input_shape = (1, self.backcast_length, self.input_dim)
        self.exo_shape = (1, self.backcast_length, self.exo_dim)
        self.output_shape = (1, self.forecast_length, self.output_dim)
        self.weights = {}
        ...
        x_input = Input(shape=self.input_shape, name='input_variable')
        x = Reshape(target_shape=(1, self.input_dim, self.backcast_length))(x_input)
        if self.has_exog():
            e_input = Input(shape=self.exo_shape, name='exos_variables')
            e = Reshape(target_shape=(1, self.exo_dim, self.backcast_length))(e_input)
        else:
            e = None
        ...
        for stack_id in range(len(self.stack_types)):
            ...
            for block_id in range(self.nb_blocks_per_stack):
                backcast, forecast = self.create_block(x, e, stack_id, block_id, stack_type, nb_poly)
                x = Add()([x, backcast])

#                 kernel_initializer_np = np.zeros((1, 1, backcast_length, backcast_length))
#                 for k in range(backcast_length):
#                     kernel_initializer_np[:, :, k, k] = -1
#                 kernel_initializer = tf.keras.initializers.constant(kernel_initializer_np)
#                 print(f'Backcast Shape: {backcast.shape}')
#                 minus_backcast = Conv2D(backcast_length, (1, 1), kernel_initializer=kernel_initializer, use_bias=False, trainable=False, name=f'{stack_id}/{block_id}/{stack_type}/Conv_layer_minus_one')(backcast)
#                 x = Add()([x, minus_backcast])

#                 minus_ones = tf.cast(tf.convert_to_tensor(- np.ones((1, self.input_dim, self.backcast_length))), dtype=tf.float32)
#                 x = Add()([x, tf.math.multiply(minus_ones, backcast)])
#                 x = Add()([x, -backcast])
#                 x = Subtract()([x, backcast])
#                 kernel_initializer = tf.keras.initializers.Constant(-1.0)
#                 layer_name = f'stack_{stack_id}-{stack_type.title()}Block_{block_id}'
#                 minus_backcast = Conv2D(1, (1, 1), kernel_initializer=kernel_initializer, use_bias=False, trainable=False, name=f'{stack_id}/{block_id}/{stack_type}/Conv_layer_minus_one')(backcast)
#                 x = Add()([x, minus_backcast])
        ...
        y = Reshape(target_shape=(1, self.forecast_length, self.input_dim))(y)
        x = Reshape(target_shape=(1, self.backcast_length, self.input_dim))(x)

        if self.input_dim != self.output_dim:
            y = Dense(self.output_dim, activation='linear', name='reg_y')(y_)

        inputs_x = [x_input, e_input] if self.has_exog() else x_input
        n_beats_forecast = Model(inputs_x, y, name=self._FORECAST)
        n_beats_backcast = Model(inputs_x, x, name=self._BACKCAST)
...
#             theta_f = reg(Dense(nb_poly, activation='linear', use_bias=False, name=n('theta_f')))
            theta_b = reg(Dense(nb_poly, activation='linear', use_bias=True, name=n('theta_b')))
            theta_f = reg(Dense(nb_poly, activation='linear', use_bias=True, name=n('theta_f')))
        ...
        backcast_ = backcast(theta_b_)
        forecast_ = forecast(theta_f_)
