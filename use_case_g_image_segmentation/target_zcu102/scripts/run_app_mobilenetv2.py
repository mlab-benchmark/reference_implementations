...
xmodelDir = os.getcwd() + "/model_dir/unet4_mobilenetv2.xmodel"
...
def run_app(numThreads, numFrames):
    ...
    inputImgDpu[2] = cpu_zero_pad(outputImgDpu[1][0], numFrames)

    #######################################################
    # DPU subgraph 2
    #######################################################

    inputs = [outputImgDpu[1][0], inputImgDpu[2]]
    ...
    inputImgDpu[3] = cpu_zero_pad(outputImgDpu[2][0], numFrames)
    ...
    inputs = [outputImgDpu[2][0], inputImgDpu[3]]
    ...
    inputImgDpu[4] = cpu_zero_pad(outputImgDpu[3][0], numFrames)

    #######################################################
    # DPU subgraph 4
    #######################################################

    inputs = [outputImgDpu[0][0], outputImgDpu[1][0], outputImgDpu[2][0], outputImgDpu[3][0], inputImgDpu[4]]
    ...
    prediction = cpu_sigmoid_calc(outputImgDpu[4][0], numFrames)
...
def main():
    ...
    with open('metrics_mobilenetv2.csv', 'w', encoding = 'UTF8') as f:
        writer = csv.writer(f)
